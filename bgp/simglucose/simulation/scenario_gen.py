...
class SemiRandomScenario(Scenario):
    ...
    def create_scenario(self):
        ...
        for tlb, tub, tbar, tsd, mbar in zip(
            time_lb, time_ub, time_mu, time_sigma, amount
        ):
            tmeal = np.round(
                truncnorm.rvs(
                    a=(tlb - tbar) / tsd,
...
def harris_benedict(weight, kind):
    # (min, max, age, weight)
    if kind == "child":
        child_weight_to_age_and_height = [
            (0, 25.6, 7, 121.9),
            (25.6, 28.6, 8, 128),
            (28.6, 32, 9, 133.3),
            (32, 35.6, 10, 138.4),
        ...
        adolescent_weight_to_age_and_height = [
            (0, 50.8, 13, 156.2),
            (50.8, 56.0, 14, 163.8),
            (56.0, 60.8, 15, 170.1),
            (60.8, 64.4, 16, 173.4),
...
class RandomBalancedScenario(Scenario):
    ...
    def get_action(self, t):
        ...
        for idx, time in enumerate(self.scenario["meal"]["time"]):
            if t_min >= time and t_min < time + self.meal_duration:
                return Action(
                    meal=self.scenario["meal"]["amount"][idx] / self.meal_duration
                )
        else:
            return Action(meal=0)
    ...
    def create_scenario_restricted(self):
        ...
        if self.kind == "child":
            ...
            raise ValueError(
                "{} not a valid kind (child, adolescent, adult)".format(self.kind)
            )
...
class CustomBalancedScenario(Scenario):
    ...
    def create_scenario(self):
        ...
        for tlb, tub, mbar, msd in zip(time_lb, time_ub, amount_mu, amount_sigma):
            tmeal = np.round(
                uniform.rvs(loc=tlb, scale=tub - tlb, random_state=self.random_gen)
            )
            ameal = max(round(self.random_gen.normal(mbar, msd)), 0)
            scenario["meal"]["time"].append(tmeal)
            scenario["meal"]["amount"].append(ameal)
