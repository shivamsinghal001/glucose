...
for k in range(n_iter):
    ...
    for person in person_grid:
        ...
        env = bgp_env.DeepSACT1DEnv(
            ...
            patient_name=person,
            seeds={"numpy": 0, "sensor": 0, "scenario": 0},
        ...
        res_arr = Parallel(n_jobs=n_jobs)(
            delayed(pid.pid_test)(
                env=env, pid=config["controller"], n_days=n_days, seed=config["seed"]
            )
    ...
    for pat in person_grid:
        ...
        for key in dat:
            for seed in range(n_seeds):
                ...
                d["euglycemic"] = np.logical_and(
                    dat[key][seed]["BG"] < 180, dat[key][seed]["BG"] > 70
                ).sum() / len(dat[key][seed]["BG"])
        ...
        for kp in df_pat["kp"].unique():
            for ki in df_pat["ki"].unique():
                for kd in df_pat["kd"].unique():
                    perf = df_pat.query(
                        "kp == {} and ki == {} and kd == {}".format(kp, ki, kd)
                    )["risk"]
    ...
    for person_name in df_pid["name"].unique():
        ...
        for k_ind in range(3):
            ...
            grid = list(
                np.sort(
                    df_pid.query('name=="{}"'.format(person_name))[k_type].unique()
                )[::-1]
            )
            ...
            for k_val in grid:
                perf_grid.append(
                    df_pid.query(
                        'name=="{}" and {}=={}'.format(person_name, k_type, k_val)
                    )["euglycemic"].mean()
                )
            patient_grid_dict[person_name][k_type] = rh.update_grid_dict(
                grid, prev_best, curr_best, n_dim, perf_grid
            )
    joblib.dump(
        (patient_grid_dict, best_setting_dict),
        "{}/{}/grid_and_settings.pkl".format(data_dir, itername),
    )
