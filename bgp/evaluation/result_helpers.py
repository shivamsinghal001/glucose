...
def rlkit_load_it_up(
    ...
    list_files = Parallel(n_jobs=n_jobs, verbose=1)(
        delayed(rlkit_load_and_summarize)(
            directory,
            f,
            itr,
            run_if_absent,
...
def rlkit_evaluate(
    directory, fl, itr, save_q, cpu_only, gpu_remap, new_eval, bgp_dir, submod, use_min
):
    """
    ...
    if save_q:
        ...
        if itr is None:
            ...
            joblib.dump(
                (q_save, adv_save), "{}/{}/q_and_adv_{}.pkl".format(directory, fl, itr)
            )
    if itr is None:
        ...
        if submod is not None:
            ...
            joblib.dump(
                simulate, "{}/simulate_best_{}.pkl".format(variant["log_dir"], submod)
            )
        else:
...
def _rlkit_load_and_summarize(
    ...
    if itr is None:
        if (
            not os.path.exists("{}/{}/simulate.pkl".format(directory, fl))
            and run_if_absent
        ):
            rlkit_evaluate(
                directory,
        ...
        if (
            not os.path.exists(
                "{}/{}/simulate_{}{}.pkl".format(directory, fl, itr, suffix)
            )
            and run_if_absent
        ):
            rlkit_evaluate(
    ...
    try:
        ...
        if itr is None:
            ...
            if eval_seed is not None:
                simulate = joblib.load(
                    "{}/{}/simulate_rerun_seed{}.pkl".format(directory, fl, eval_seed)
                )
            else:
                if submod is not None:
                    res["submod"] = submod
                    simulate = joblib.load(
                        "{}/{}/simulate_{}_{}{}.pkl".format(
                            directory, fl, itr, submod, suffix
                        )
                    )
                else:
                    simulate = joblib.load(
                        "{}/{}/simulate_{}.pkl".format(directory, fl, itr)
                    )
        if simulate is -1:
            return {}
        if itr is None:
            ...
            res[
                "itr"
            ] = "-2"  # TODO: could replace with last observed itr, too much trouble for now
        else:
            if "best" in itr:
                if itr == "best":
        ...
        for pair in pairs:
            ...
            if progress_name in progress:
                ...
                try:
                    ...
                    res["{}_max_ind".format(label_name)] = progress[
                        progress_name
                    ].idxmax()
                except:
...
def rlkit_load_and_summarize(
    ...
    if n_eval_runs is None:
        return _rlkit_load_and_summarize(
            directory,
            fl,
            itr,
            run_if_absent,
