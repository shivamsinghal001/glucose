...
class Logger(object):
    ...
    def save_itr_params(self, itr, params, best=False):
        if best:
            ...
            torch.save(
                {
                    "policy": policy_cache.state_dict(),
                    "qf": qf_cache.state_dict(),
                    "vf": vf_cache.state_dict(),
                    "target_vf_cache": target_vf_cache.state_dict(),
                },
        ...
        if self._snapshot_dir:
            if self._snapshot_mode == "all":
                ...
                if itr % self._snapshot_gap == 0:
                    file_name = osp.join(self._snapshot_dir, "itr_%d.pkl" % itr)
                    joblib.dump(params, file_name)
                    eval_file_name = osp.join(
                        self._snapshot_dir, "eval_itr_%d.pkl" % itr
                    )
                    joblib.dump(params["eval_policy"], eval_file_name)
                ...
                if itr % self._snapshot_gap == 0:
                    ...
                    if mdl.stochastic_policy.features is not None:
                        ...
                        mdl.stochastic_policy.features = (
                            mdl.stochastic_policy.features.to(run_device)
                        )
                    ...
                    if mdl.stochastic_policy.features is not None:
                        ...
                        mdl.stochastic_policy.features = (
                            mdl.stochastic_policy.features.to(orig_run_device)
                        )
                    cache_file_name = osp.join(
                        self._snapshot_dir, "cache_itr_%d.pkl" % itr
                    )
                    ...
                    joblib.dump(
                        {
                            ...
                            "qf": qf_cache,
                            "vf": vf_cache,
                            "target_vf": target_vf_cache,
                            "alpha": alpha_cache,
                        },
                        cache_file_name,
                    )
                    optim_file_name = osp.join(
                        self._snapshot_dir, "optim_itr_%d.pkl" % itr
                    )
