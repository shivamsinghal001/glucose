...
class ReplayMemory:
    ...
    def _get_sample_from_segment(self, segment, i):
        ...
        while not valid:
            ...
            if (
                (self.transitions.index - idx) % self.capacity > self.n
        ...
        state = (
            torch.stack([trans.state for trans in transition[: self.history]])
        ...
        R = torch.tensor(
            [
                sum(
                    self.discount**n * transition[self.history + n - 1].reward
    ...
    def __next__(self):
        ...
        for t in reversed(range(self.history - 1)):
            if prev_timestep == 0:
                ...
                state_stack[t] = self.transitions.data[
                    self.current_idx + t - self.history + 1
                ].state
                prev_timestep -= 1
        state = (
            torch.stack(state_stack, 0)
            .to(dtype=torch.float32, device=self.device)
            .div_(255)
