...
class SoftActorCritic(TorchRLAlgorithm):
    def __init__(
        ...
        **kwargs
        ...
        if eval_deterministic:
            ...
            eval_policy = policy
        super().__init__(
            env=env, exploration_policy=policy, eval_policy=eval_policy, **kwargs
        )
        self.policy = policy
        self.qf = qf
        self.vf = vf
        ...
        if self.use_automatic_entropy_tuning:
            ...
            self.alpha_optimizer = optimizer_class(
                [self.log_alpha], lr=policy_lr, weight_decay=weight_decay
            )
        ...
        self.policy_optimizer = optimizer_class(
            self.policy.parameters(), lr=policy_lr, weight_decay=weight_decay
        )
        self.qf_optimizer = optimizer_class(
            self.qf.parameters(), lr=qf_lr, weight_decay=weight_decay
        )
        self.vf_optimizer = optimizer_class(
            self.vf.parameters(), lr=vf_lr, weight_decay=weight_decay
        )
    ...
    def _do_training(self):
        ...
        q_target = rewards + (1.0 - terminals) * self.discount * target_v_values
        ...
        if self.need_to_update_eval_statistics:
            ...
            for param in self.qf.parameters():
                try:
                    qf_grads = torch.cat(
                        (qf_grads, torch.abs(param.grad.data.flatten()))
                    )
                except:
            ...
            for param in self.vf.parameters():
                try:
                    vf_grads = torch.cat(
                        (vf_grads, torch.abs(param.grad.data.flatten()))
                    )
                except:
                    pass  # seems to be a weird error on mld5 around layernorm
            ...
            for param in self.policy.parameters():
                try:
                    policy_grads = torch.cat(
                        (policy_grads, torch.abs(param.grad.data.flatten()))
                    )
                except:
                    pass  # seems to be a weird error on mld5 around layernorm
    ...
    def get_epoch_snapshot(self, epoch):
        ...
        if self.use_automatic_entropy_tuning:
            snapshot.update(alpha=self.log_alpha, alpha_optim=self.alpha_optimizer)
        return snapshot
