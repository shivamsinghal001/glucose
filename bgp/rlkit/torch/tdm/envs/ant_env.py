...
from bgp.rlkit.envs.ant import AntEnv
...
class GoalXYPosAnt(AntEnv, MultitaskEnv, Serializable):
    ...
    def step(self, action):
        ...
        info_dict["desired_dist_from_origin"] = np.linalg.norm(self.multitask_goal)
...
class GoalXYPosAndVelAnt(AntEnv, MultitaskEnv, Serializable):
    ...
    def sample_goals(self, batch_size):
        return np.random.uniform(
            ...
            np.array(
                [self.max_distance, self.max_distance, self.max_speed, self.max_speed]
            ),
            (batch_size, 4),
        )
    ...
    def log_diagnostics(self, paths, logger=default_logger):
        ...
        weighted_error = get_stat_in_paths(
            paths, "env_infos", "weighted_pos_error"
