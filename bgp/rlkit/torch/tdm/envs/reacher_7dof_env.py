...
class Reacher7DofMultitaskEnv(MultitaskEnv, mujoco_env.MujocoEnv, Serializable):
    ...
    def _step(self, a):
        distance = np.linalg.norm(self.get_body_com("tips_arm") - self._desired_xyz)
        reward = -distance
    ...
    def log_diagnostics(self, paths, logger=default_logger):
        ...
        statistics.update(
            create_stats_ordered_dict("Euclidean distance to goal", euclidean_distances)
        )
        statistics.update(
...
class Reacher7DofFullGoal(Reacher7DofMultitaskEnv):
    ...
    def sample_states(self, batch_size):
        ...
        random_xyz = np.random.uniform(
            np.array([-0.75, -1.25, -0.2]), np.array([0.75, 0.25, 0.6]), (batch_size, 3)
        )
        return np.hstack(
            (
                random_pos,
    ...
    def cost_fn(self, states, actions, next_states):
        ...
        desired_joint_angles = self.multitask_goal[:7] * np.ones_like(next_joint_angles)
