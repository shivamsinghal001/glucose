...
from bgp.rlkit.torch.sac.policies import (
    CNNTanhGaussianPolicy,
    GRUTanhGaussianPolicy,
    TanhGaussianPolicy,
    FancyCNNTanhGaussianPolicy,
)
...
def get_best_itr(variant, max=None, seed=None, use_min=True):
    ...
    try:
        if seed is None:
            ...
            progress = pd.read_csv(
                "{}/progress_seed{}.csv".format(variant["log_dir"], seed)
            )
    ...
    if len(eval_itrs) > 0:
        ...
        cached_itrs = sorted(
            list(map(lambda x: int(x.split(".")[0].split("_")[2]), eval_itrs))
        )
    else:
        # old method, should only have itr_x.pkl
        cached_itrs = sorted(
            list(map(lambda x: int(x.split(".")[0].split("_")[1]), files))
        )
    cached_itrs = list(
        filter(lambda x: x >= 0 and x <= list(progress.index)[-1], cached_itrs)
    )
    if max is not None:
...
def simulate_policy(
    ...
    if itr is None:
        ...
        if itr == "best":
            ...
            joblib.dump(
                best_itr, "{}/best_itr={}.pkl".format(variant["log_dir"], best_itr)
            )
            if best_itr == -1:
                # Nothing has actually run
                return -1
            data = joblib.load(
                "{}/{}itr_{}.pkl".format(variant["log_dir"], prefix, best_itr)
            )
        else:
            ...
            joblib.dump(
                best_itr,
                "{}/best_itr<{}={}.pkl".format(
                    variant["log_dir"], best_itr_max, best_itr
                ),
            )
            if best_itr == -1:
                # Nothing has actually run
    ...
    if variant["model_type"] == "sac":
        ...
        if mdl.stochastic_policy.features is not None:
            ...
            mdl.stochastic_policy.features = mdl.stochastic_policy.features.to(
                run_device
            )
            mdl.stochastic_policy.features.eval()
        env = bgp_env.DeepSACT1DEnv(
            reward_fun=reward_fun,
            patient_name=variant["patient_name"],
            seeds={
                "numpy": variant["base_seed"]
                + variant["sim_seed_mod"]
                + sim_seed_submod,
            ...
            reset_lim=variant["reset_lim"],
            time=variant["include_time"],
            meal=variant["include_meal"],
            bw_meals=variant["bw_meals"],
            load=variant["load"],
            use_pid_load=variant["use_pid_load"],
            hist_init=variant["hist_init"],
            gt=variant["use_ground_truth"],
            n_hours=variant["n_hours"],
            norm=variant["norm"],
            time_std=variant["time_std"],
            use_old_patient_env=variant["use_old_patient_env"],
            action_cap=variant["action_cap"],
            action_bias=variant["action_bias"],
            action_scale=variant["action_scale"],
            basal_scaling=variant["basal_scaling"],
            meal_announce=variant["meal_announce"],
            residual_basal=variant["residual_basal"],
            residual_bolus=variant["residual_bolus"],
            residual_PID=variant["residual_PID"],
            fake_gt=variant["fake_gt"],
            fake_real=variant["fake_real"],
            suppress_carbs=variant["suppress_carbs"],
            limited_gt=variant["limited_gt"],
            ...
            restricted_carb=variant["restricted_sched"],
            meal_duration=variant["meal_duration"],
            rolling_insulin_lim=variant["rolling_insulin_lim"],
            universal=variant["universal"],
            reward_bias=variant["reward_bias"],
            carb_error_std=variant["carb_error_std"],
            carb_miss_prob=variant["carb_miss_prob"],
            source_dir=variant["source_dir"],
            noise_scale=variant["noise_scale"],
            true_reward_fn=true_reward_fun,
        )
    else:
    ...
    for _ in tqdm(range(variant["n_sim_days"] * 288)):
        if save_q:
            if variant["rnn"]:  # TODO: not sure about this
                input = torch.tensor(
                    env.get_state(variant["norm"]), device=run_device
                ).float()[:, None]
            else:
                input = torch.tensor(
                    env.get_state(variant["norm"]), device=run_device
        ...
        try:
            ...
            mdl.stochastic_policy = mdl.stochastic_policy.to(run_device)
            raise ValueError(
                mdl.stochastic_policy.features.device,
                run_device,
...
def run_em_sac(variant):
    ...
    if variant["use_ground_truth"]:
        ...
        vf = FancyCNNQ(input_size=obs_dim, output_size=1, device=variant["device"])
        policy = FancyCNNTanhGaussianPolicy(
            obs_dim=obs_dim, action_dim=action_dim, device=variant["device"]
        )
    elif variant["rnn"]:
        qf = SimpleGRUQ(
            input_size=[obs_dim[0] + 1, obs_dim[1]],
        ...
        vf = SimpleCNNQ(input_size=obs_dim, output_size=1, device=variant["device"])
    ...
    if variant["warm_start"] is not None:
        ...
        if "{}.pkl".format(variant["patient_name"]) in files:
            param_dict = joblib.load(
                "/data/dir/{}/{}.pkl".format(
                    variant["warm_start"], variant["patient_name"]
                )
            )
        elif "{}.pkl".format(variant["patient_name"].split("#")[0]) in files:
            param_dict = joblib.load(
                "/data/dir/{}/{}.pkl".format(
                    variant["warm_start"], variant["patient_name"].split("#")[0]
                )
            )
        elif "{}#001.pkl".format(variant["patient_name"].split("#")[0]) in files:
            param_dict = joblib.load(
                "/data/dir/{}/{}#001.pkl".format(
                    variant["warm_start"], variant["patient_name"].split("#")[0]
                )
            )
        else:
            ...
            param_dict = joblib.load(
                "/data/dir/{}/full.pkl".format(variant["warm_start"])
            )
...
def finish_sac(variant):
    ...
    for submod in range(variant["num_eval_runs"]):
        ...
        hist = simulate_policy(
            variant,
            itr=variant["finish_itr"],
            sim_seed_submod=submod,
            use_min=variant["use_min"],
        )
        if submod == 0:
            # some backwards compatibility
            joblib.dump(
                hist, "{}/simulate_best{}.pkl".format(variant["log_dir"], suffix)
            )
        joblib.dump(
            hist, "{}/simulate_best_{}{}.pkl".format(variant["log_dir"], submod, suffix)
        )
